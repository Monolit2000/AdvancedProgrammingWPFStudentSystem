    ...
    public class AthleticsFacilityTests
    {
        ...
        public void Remove_StudentOrganization_With_Valid_Id()
        {
            using (var context = new UniversityContext(_options))
            {
                ...
                var viewModel = new AthleticsFacilitysViewModel(context, dialogService);
                var initialCount = context.AthleticsFacilities.Count();

                viewModel.Remove.Execute((long)1); 

                var athleticsFacilitieExists = context.AthleticsFacilities.Any(org => org.AthleticsFacilityId == 1);
                Assert.IsFalse(athleticsFacilitieExists);
                Assert.AreEqual(initialCount - 1, context.AthleticsFacilities.Count());
            }
        }
        ...
        public void Remove_StudentOrganization_With_Invalid_Id()
        {
            using (var context = new UniversityContext(_options))
            {
                ...
                var athleticsFacilitieExists = context.AthleticsFacilities.Any(org => org.AthleticsFacilityId == 9999);
                Assert.IsFalse(athleticsFacilitieExists); 
                Assert.AreEqual(initialCount, context.AthleticsFacilities.Count()); 
            }
        }
